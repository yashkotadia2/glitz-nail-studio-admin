...
export const isTimeOutsideWorkingHours = (time: string): boolean => {
  // Set default timezone (Asia/Calcutta or whichever timezone you prefer)
  const timezoneToUse = "Asia/Calcutta";

  // Get start and end time from environment variables, default to fallback if not set
  const startTime = process.env.START_TIME || "09:00"; // Default start time: 9:00 AM
  const endTime = process.env.END_TIME || "18:00"; // Default end time: 6:00 PM

  // Convert the times to dayjs objects with timezone for comparison
  const workingStartTime = dayjs.tz(startTime, "HH:mm", timezoneToUse);
  const workingEndTime = dayjs.tz(endTime, "HH:mm", timezoneToUse);
  const inputTime = dayjs.tz(time, timezoneToUse);
  ...
  // Check if the time is outside the working hours (inclusive)
  return !inputTime.isBetween(workingStartTime, workingEndTime, "minute", "[]");
};
